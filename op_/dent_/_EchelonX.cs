...
namespace nilnul.num.real.matrix.op_.dent_
{
	...
	static public class _EchelonX
	{
			...
			for (var col = 0; col < cols; col++)
			{
				for (var row = rank; row < rows; row++)
				{
					...
					if (head != 0)
					{
						...
						_indent._GaussGramX._Shed_2divisor(ref m, rank, head); /// make it <see cref="num.real.vec_.IStd"/>
		...
		static public long EchelonForLongRank(ref Q[,] m)
		{
			// get the first col
			// long col = 0;
			//	long row = 0;
