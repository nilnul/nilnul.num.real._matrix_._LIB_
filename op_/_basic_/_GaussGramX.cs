...
namespace nilnul.num.real.matrix.op_._indent
{
	...
	static public class _GaussGramX
	{
		...
		[Obsolete(nameof(op_.basic_._SkewX))]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		static public void _Skew_12row(ref double[,] m, int augend, int addend, double scale)
		{
			matrix.op_.basic_._SkewX._Skew_12row(ref m, augend, addend, scale);
			return;
		...
		{
			matrix.op_.basic_._SkewX._Skew_12row_3nilish(ref m, augend, addend, scale);
			return;
			//if (scale==0)
			//{
			//	return;
		...
		[Obsolete("we require swap on two different rows")]
