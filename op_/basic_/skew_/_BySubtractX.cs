...
namespace nilnul.num.real.matrix.op_.basic_.skew_
{
	/// <summary>
	///  r[i] -= r[j]*x
	///  ;
	///  for -, we can not use <see cref="matrix_.sq_.basic_.ISkew"/> where it's a matrix for multiplication (division is complex, unassociative), but we can use <see cref="matrix.IOp"/>, where we can explicitly name the input parameter as subtractor:<see cref="op_.basic_.skew_"/>.
	///  ; note the two rows must be different to make the op unregressed;
	/// </summary>
	/// <remarks>
	///  -= is the inverse of '+='
	///  the <see cref="matrix_.sq_.basic_._SkewX"/> is:
	///		[
	///			-k, 1
	///		]
	/// </remarks>
