...
namespace nilnul.num.real.matrix.op_.unary_.elementary_.rowwise_
{
	...
	/// </summary>
	/// 
	[Obsolete(nameof(real.matrix.op_._ScaleX) + " means scale each cel, but here it scale only one row; so it's better renamed as " + nameof(nilnul.num.real.matrix.op_._indent.gauss_.IShed))]
