...
namespace nilnul.num.real.matrix_.rowly_.indep
{
    static public class _PseudoInverseX
    {
		...
		/// </summary>
		/// <param name="x">
		/// rowly independent matrix.
		/// </param>
		/// <returns>
		/// of size [cols, rows  ];
		/// the right inverse. that is, let y be the returned, then
		/// xy = I[rows, rows]
		/// </returns>
		static public double[,] _PseudoInverse_0rowlyIndep(double[,] x) {
