...
namespace nilnul.num.real.matrix_.vent.op_
{
	...
	static public class _DedundantX
	{
		...
		static public Q[,] ToIndepVent( ref Q[,] matrix)
		{
			var _rank = real.matrix.op_. dent_._EchelonX.EchelonForRank(ref matrix); /// we need the rank twice

			real.matrix.op_.dent_.echelon_._VentX._Vent_0echelon_1rank(ref matrix, _rank );

			return real.matrix. op_.dent_._DedundantX._ToDedundant_0indent_1rank(matrix, _rank);

		}


		static public Q[,] ToIndepVent(  Q[,] matrix)
		{
