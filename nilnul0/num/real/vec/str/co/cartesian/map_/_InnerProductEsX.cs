...
namespace nilnul.num.real.vec.str.co.cartesian.map_
{
	///
	...
	///		by split
	///		innerProductEs
	///			map the str to a str by mapping. So Es is appended to explicate that this is a mapping of str.
	///


	static public class _InnerProductEsX
	{
		...
		{
			return major.Select(
				x => str.map_._InnerProductX._MultiAsRow_colAssumeEquisize2row(x, minor_cols)
			);
		}
		...
		static public double[,] _MultiAsMatrix_vecsAssumeEquisize(
			IEnumerable<IEnumerable<double>> major
			,
			IEnumerable<IEnumerable<double>> minor
		)
		{
			var r = _Multi_vecsAssumeEquisize(major, minor);

			if (r.Any())
			{
				return obj.matrix.of_.vecs_._OfRowsX._OfVecs_assumeSameArity(r);
			}
			return new double[0, minor.Count()];
		}


		static public IEnumerable<IEnumerable<double>> _Multi_assumeMultible(this double[,] a, double[,] b)
		{
			return _Multi_vecsAssumeEquisize(
				obj.matrix._RowsX.Rows(a)
				,
				obj.matrix._ExtensionsX.Cols(b)
			);
		}
	}
}
