...
namespace nilnul.num.real.vec.compart.str.co.cartesian.map_
{
	...
	///	by grouping
	///	by compart
	///	<see cref="nilnul.num.real.matrix.vec.co_.sommable._Belt1bunchX"/>
	///
	[Obsolete("the definition is hard to paraphrase")]
	public class _InnerProductEsX
	{
		...
		)
		{
			return belt.Select(
				r=>
				bunch.Select(
					c=> num.real.vec.compart.co._InnerProductX._InnerProduct_assumeComparts(r,c) /// this requires the two inputs are antisize.
				)
			);
		}
		...
		)
		{
			...
			if (r1.Any())
			{
				return obj.matrix.of_.vecs_._OfRowsX._OfVecs_assumeSameArity(r1);  /// take each group( traces) as a row ? 
			}
