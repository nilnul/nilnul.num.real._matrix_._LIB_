...
namespace nilnul.num.real.matrix.vec.co_.sommable
{
	...
	///  alias:
	///		som
	///		sop,where p might mean multiple terms

	static public class _SomX
	{
		...
		/// eg:
		/// 
		static public double[,] _Som_01sommable(
		...
		)
		{
			...
			var r = co_.multible._MultiComponentwiseX._Multi_assumeComponentwiseMultible(belt, bunch);
		...
		static public Q[,] _Som_01sommable(
			IEnumerable<Q[,]> belt
			,
			IEnumerable<Q[,]> bunch
		)
		{
			...
			{
				return r.Aggregate(
					(a1,c) => matrix.co_.plusable._PlusX._Add_1plusable(a1,c)
				);
			}
			return new Q[0, 0];
